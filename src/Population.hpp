...
inline void Population<T,N...>::select(int pos)
{
   #ifndef NDEBUG
   if (pos > ptr->popsize - 1) {
      throw std::out_of_range("Error: in galgo::Population<T>::select(int), exceeding current population memory.");
   }
   if (matidx == ptr->matsize) {
      throw std::out_of_range("Error: in galgo::Population<T>::select(int), exceeding mating population memory.");
   }
   #endif
