...
Chromosome<T,N...>::setGene(int k) 
{
   ...
   if (k < 0 || k >= ptr->nbparam) {
      throw std::out_of_range("Error: in galgo::Chromosome<T>::setGene(int), argument cannot be outside interval [0,nbparam-1], please amend.");
   }
   #endif
...
Chromosome<T,N...>::initGene(int k, T x)
{
   #ifndef NDEBUG
   if (k < 0 || k >= ptr->nbparam) {
      throw std::out_of_range("Error: in galgo::Chromosome<T>::initGene(int), first argument cannot be outside interval [0,nbparam-1], please amend.");
   }
   #endif
